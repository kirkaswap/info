  ...
  const ListItem = ({ item }) => {
    return (
      <DashGrid style={{ height: '48px' }}>
        ...
        <DataText area="value">
          {currency === 'pCKB' ? 'Ξ ' + formattedNum(item.valueETH) : formattedNum(item.amountUSD, true)}
        </DataText>
        {!below780 && (
          <>
        ...
        {!below1080 && (
          <DataText area="account">
            <Link color={color} external href={'https://gw-testnet-explorer.nervosdao.community/address/' + item.account}>
