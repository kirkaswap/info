      ...
      <ContentWrapper>
        <RowBetween>
          <TYPE.body>
            ...
            <Link lineHeight={'145.23%'} href={'https://gw-testnet-explorer.nervosdao.community/address/' + account} target="_blank">
              {' '}
              {account?.slice(0, 42)}{' '}
            </Link>
          </TYPE.body>
            ...
            <DropdownWrapper>
              ...
              {showDropdown && (
                <Flyout>
                  <AutoColumn gap="0px">
                    {positions?.map((p, i) => {
                      if (p.pair.token1.symbol === 'WpCKB') {
                        p.pair.token1.symbol = 'pCKB'
                      }
                      if (p.pair.token0.symbol === 'WpCKB') {
                        p.pair.token0.symbol = 'pCKB'
                      }
                      return (
